...
public class ErrorHandler {
    ...
    public ApiError handleDataValidationException(DataValidationException e, HttpStatus status) {
        ...
        e.printStackTrace(new PrintWriter(out));
        String stackTrace = out.toString();
        return new ApiError(status, "Ошибка в поступивших данных", e.getMessage(),
                Collections.singletonList(stackTrace), LocalDateTime.now());
    }
    ...
    @ExceptionHandler
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public ApiError handleUserNotFoundException(UserNotFoundException e, HttpStatus status) {
        log.error("Ошибка поиска пользователя");
        StringWriter out = new StringWriter();
        e.printStackTrace(new PrintWriter(out));
        String stackTrace = out.toString();
        return new ApiError(status, "Ошибка поиска пользователя", e.getMessage(),
                Collections.singletonList(stackTrace), LocalDateTime.now());
    }
